		...
		},
		{
			...
			render: (text, record) => (
				<div>
					...
					<Button icon={<EditOutlined />} style={{ margin: 8 }} type="primary" onClick={() => message.info(`Edit sertifikat dengan ID ${record._id}`)} />
					<Button icon={<DownloadOutlined />} type="primary" style={{ margin: 8 }} onClick={() => message.info(`Unduh sertifikat dengan ID ${record._id}`)} />
				</div>
			),
		},
	];
	...
	return (
		<MainLayout>
			<div className="flex flex-col items-center justify-center w-full lg:w-3/4 p-5">
				<div>
					<p className="text-xl font-Poppins font-semibold mb-5 text-Text p-3 bg-white rounded-xl">History list</p>
				</div>
