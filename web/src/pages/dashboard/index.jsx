import { Table, Button, Modal, message, Row, Col, Form, Input, DatePicker, InputNumber, Select } from 'antd';
...
const Dashboard = () => {
	...
	const onSubmit = async (formData) => {
		try {
			const sanitizedHardSkills = (formData.hardSkill || []).filter((skill) => skill.skill_name && skill.jp);
			const sanitizedSoftSkills = (formData.softSkill || []).filter((skill) => skill.skill_name && skill.skill_score);
	...
	const filteredData = dta.filter((item) => item.sertif_name.toLowerCase().includes(searchText.toLowerCase()));

	const downloadPDF = async (_id) => {
		try {
			const response = await Sertifikat.get(`/download/${_id}/a`, {
				headers: {
	...
	const columns = [
		...
		},
		{
			...
			render: (text, record) => (
				<div>
					<Button icon={<DeleteOutlined />} style={{ margin: 8 }} type="primary" danger onClick={() => showDeleteConfirm(record._id)} />
					<Button icon={<EditOutlined />} style={{ margin: 8 }} type="primary" onClick={() => handleEdit(record)} />
					<Button icon={<DownloadOutlined />} type="primary" style={{ margin: 8 }} onClick={() => downloadPDF(record.data_id)} />
				</div>
			),
		},
	];
					...
					afterOpenChange={(visible) => {
						if (visible && currentRecord) {
							reset({
								...
								validTime: currentRecord?.valid_date?.valid_total || 'Tidak mengisi',
								expiredTimeStart: currentRecord?.valid_date?.valid_start || 'Tidak mengisi',
								expiredTimeEnd: currentRecord?.valid_date?.valid_end || 'Tidak mengisi',
								totalMeeting: currentRecord?.total_meet || 'Tidak mengisi',
								meetingTime: currentRecord?.meet_time || 'Tidak mengisi',
								selectedCompetenceId: currentRecord?.kompeten_bidang || 'Tidak mengisi', // Atur nilai awal di sini
								selectedSignatureId: currentRecord?.kompeten_bidang || 'Tidak mengisi',
					...
					>
						...
						<Form.Item label="Nama Sertifikat" required>
							<Controller
								...
								render={({ field }) => <Input {...field} placeholder="Masukkan nama sertifikat" style={{ width: '100%', height: '50px' }} />}
							/>
						</Form.Item>

						<Form.Item label="Nama" required>
							<Controller name="nama" control={control} rules={{ required: 'Nama peserta diperlukan' }} render={({ field }) => <Input {...field} placeholder="Masukkan nama peserta" style={{ width: '100%', height: '50px' }} />} />
						</Form.Item>

						<Form.Item label="Bidang Studi" required>
							<Controller name="fieldOfStudy" control={control} rules={{ required: 'Bidang studi diperlukan' }} render={({ field }) => <Input {...field} placeholder="Masukkan bidang studi" style={{ width: '100%', height: '50px' }} />} />
						</Form.Item>

						<Form.Item label="Total Tahun" required>
							<Controller name="validTime" control={control} rules={{ required: 'Waktu validasi diperlukan' }} render={({ field }) => <Input {...field} placeholder="Masukkan jumlah tahun" style={{ width: '100%', height: '50px' }} />} />
						</Form.Item>
						...
						<Form.Item label="Total Pertemuan" required>
							<Controller
								...
								render={({ field }) => <InputNumber {...field} placeholder="Masukkan jumlah pertemuan" style={{ width: '100%', height: '50px' }} />}
							/>
						</Form.Item>

						<Form.Item label="Waktu Pertemuan" required>
							<Controller name="meetingTime" control={control} rules={{ required: 'Waktu pertemuan diperlukan' }} render={({ field }) => <Input {...field} placeholder="2 Bulan" style={{ width: '100%', height: '50px' }} />} />
						</Form.Item>
						...
						{hardSkillFields.length > 0 && (
							<div>
								<h2 className="font-Poppins text-2xl font-medium text-center p-6">Hardskills</h2>
						...
						{softSkillFields.length > 0 && (
							<div>
								<h2 className="font-Poppins text-2xl font-medium text-center p-6">Softskills</h2>
						...
						<Form.Item required>
							<Controller
								...
								control={control}
								render={({ field }) => (
									<Select placeholder="Pilih tanda tangan" {...field} style={{ width: '100%', height: '50px' }}>
										<Option value="" disabled>
											Pilih Tanda Tangan
										</Option>
