  ...
  const Dashboard = () => {
    ...
    const { fields: hardSkillFields, replace: replaceHardSkill } =
      useFieldArray({
        control,
        name: "hardSkill",
      });

    const { fields: softSkillFields, replace: replaceSoftSkill } =
      useFieldArray({
        control,
        name: "softSkill",
      });
    ...
    const downloadPDF = async (_id) => {
      try {
        const response = await Sertifikat.get(
          `http://127.0.0.1:3000/api/certificate/${_id}`,
          {
            responseType: "blob", // Pastikan respons berupa file binary
          }
        );
        ...
        link.href = url;
        link.setAttribute("download", `${id}.pdf`); // Nama file saat diunduh
